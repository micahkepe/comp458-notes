...
\dfn{Measurement and Superposition Collapse}{When a qubit is measured in the
  computational basis $\{|0\rangle, |1\rangle\}$, it collapses to one of the
  basis states with probability:
  \[
...
\dfn{Important Quantum States}{Several quantum states are particularly
  important in quantum computing:
  \begin{itemize}
    \item \textbf{Computational Basis States:} $|0\rangle$ and $|1\rangle$
    \item \textbf{Plus/Minus States:}
      \[
        |+\rangle = \frac{1}{\sqrt{2}}(|0\rangle + |1\rangle), \quad |-\rangle =
        \frac{1}{\sqrt{2}}(|0\rangle - |1\rangle)
      \]
    \item \textbf{Complex Superposition States:}
      \[
        |i\rangle = \frac{1}{\sqrt{2}}(|0\rangle + i|1\rangle), \quad |-i\rangle
        = \frac{1}{\sqrt{2}}(|0\rangle - i|1\rangle)
      \]
\end{itemize}}

\ex{Example: Equal Superposition State}{
  A qubit initially in state $|0\rangle$ is transformed into an equal
  superposition using the Hadamard gate:
  \[
    H |0\rangle = \frac{1}{\sqrt{2}} (|0\rangle + |1\rangle).
  \]
  Measuring this state results in either $|0\rangle$ or $|1\rangle$ with equal
  probability $P(0) = P(1) = \frac{1}{2}$.
...
\dfn{Quantum Gates and Operations}{Quantum gates are unitary matrices that
  transform qubits. A general qubit transformation is given by:
  \[
...
\dfn{Pauli Matrices and Gates}{The \textbf{Pauli matrices} define fundamental
  ...
  \end{itemize}
  Each of these matrices is both \textbf{Hermitian} ($A = A^\dagger$) and
...
\dfn{Circuit Notation}{Quantum circuits visually represent quantum
  ...
  \begin{itemize}
    ...
    \item \textbf{Time flow:} Left to right in circuits (opposite of matrix
      multiplication order)
    \item \textbf{Initial states:} Usually started in $|0\rangle$ unless
      specified otherwise
\end{itemize}}
...
\dfn{Measurement in Quantum Circuits}{Measurement collapses a quantum state
  to a basis state with probabilities determined by the squared magnitudes of
  its coefficients. For a state $|\psi\rangle = \alpha|0\rangle + \beta|1\rangle$:
  \begin{itemize}
    ...
    \item Multiple measurements of identically prepared states give
