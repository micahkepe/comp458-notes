...
\begin{figure}[h]
  ...
  \begin{tikzpicture}
    ...
    \node[align=center] at (box.center) {
        ...
        Quantum circuit prepares state $\ket{\psi(\gamma, \beta)}$\\
        Classical optimizer adjusts $\gamma, \beta$ to minimize\\
...
\ex{Simplified Max-Cut Circuit ($P=1$)}{
...
Here's a complete QAOA circuit for the 4-qubit Max-Cut with $P=1$ using
...
\aside{
  ...
  but practical limits (e.g., noise, coherence time) often restrict $P$ to small values in current hardware.
}
